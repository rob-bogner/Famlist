...
Pixelgenaue SwiftUI-Umsetzung aller 33 Screens aus dem Canvas „My List – Redesign“, jeweils in Light und Dark. Die Kamera-Screens gibt es nur dunkel.

Die Zuordnung „Artboard → View“ steht in `../SPEC.md` §2. Die Vorschauen liegen je Gruppe unter `Previews/`.

## Dateien

```
Support/     CSSRendering (Farben, Verläufe, box-shadow, Rahmen), AppFont (Variable Fonts), SVGIcons (Originalpfade + Parser)
Theme/       Theme.swift: Farb- und Schattenwerte, Akzent-Mathematik
Components/  SheetComponents, Dock (untere Leiste, 7 Zustände), OverlayComponents (Popover, Scrim, Toast)
Screens/     ListScreen, OverlayScreens, SearchScreen, ItemFormScreens, ProductImageScreen, ItemExtraScreens,
             MyListsScreen, ListManagementScreens, AccountScreens, OnboardingScreens, CategoryScreens, ReceiptScreens
Previews/    eine Vorschau-Datei je Gruppe, alle 390 × 844, Light + Dark
```

Hinweis: Der Code wurde ohne Xcode erstellt. Syntax und Schnittstellen zwischen den Dateien sind geprüft, der erste Build kann trotzdem kleine Compilerfehler zeigen.
...
- **Kopf:** Titelzeile → 20 → „1 Liste“ 13/600 in Großbuchstaben → 10 → Listen-Karten mit 10 Abstand.
- **Karte:** 76 hoch, Radius 22, Rahmen 1,5, Inhalt 14 eingerückt. Links Kachel 48 (Verlauf, Listen-Icon 22), dann Name Outfit 17/600 mit Stern 16 (#F5B521), darunter „n Artikel“ 13. Rechts das Auswahl-Häkchen 32.
- **Aktive Liste:** Rahmen in Akzent mit 4-pt-Ring. Andere Listen haben einen neutralen Rahmen und kein Häkchen.
...
Der Code ist ohne Xcode entstanden. SwiftUI lässt sich in dieser Umgebung nicht kompilieren. Bitte zuerst in Xcode bauen und die 18 Vorschauen gegen die Artboards legen.
