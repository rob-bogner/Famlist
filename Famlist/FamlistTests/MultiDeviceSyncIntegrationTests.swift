...
final class MultiDeviceSyncIntegrationTests: XCTestCase {
    ...
    func testConcurrentModifications_shouldResolveConsistently() async {
        ...
        // Device 1 receives Device 2's update → remote wins (Device 2 is newer)
        XCTAssertTrue(resolver.shouldApplyRemote(localMeta: device1Meta, remoteMeta: device2Meta))

        // Device 2 receives Device 1's update → local wins (Device 2 is newer)
        XCTAssertFalse(resolver.shouldApplyRemote(localMeta: device2Meta, remoteMeta: device1Meta))
    }
    ...
    func testRapidConcurrentUpdates_shouldConverge() async {
        ...
        // Each device resolves against all others — the winner must always be device4 (highest HLC)
        for deviceIndex in 0..<5 {
            var currentMeta = metas[deviceIndex]

            for otherIndex in 0..<5 where otherIndex != deviceIndex {
                if resolver.shouldApplyRemote(localMeta: currentMeta, remoteMeta: metas[otherIndex]) {
                    currentMeta = metas[otherIndex]
                }
            }

            XCTAssertEqual(currentMeta.lastModifiedBy, "device4",
                           "All devices must converge to device4 (highest HLC)")
        }
    }
